...
/*LGFX_ESP32_2432S028.hpp ESP32-2432S** に向け調整中 */
...
//lgfx::Panel_ILI9163     _panel_instance;
  lgfx::Panel_ILI9341     _panel_instance;	//ESP32-2432S**
...
lgfx::Touch_XPT2046          _touch_instance;	//ESP32-2432S**
...
//*/
    ...
    { // 表示パネル制御の設定を行います。
      ...
      cfg.offset_y         =     0;  // パネルのY方向オフセット量 ESP32_2432S028?!
      cfg.offset_rotation  =     4;  // 回転方向の値のオフセット 0~7 (4~7は上下反転) ｛ESP32-2432S** は文字表示が反転で(4~7は上下反転)を選択」
...
// 以下はST7735やILI9163のようにピクセル数が可変のドライバで表示がずれる場合にのみ設定してください｛ESP32-2432S** もピクセル数要再調整｝
...
//*/
    ...
    { // タッチスクリーン制御の設定を行います。（必要なければ削除）
      ...
      cfg.x_min      = 200;		// タッチスクリーンから得られる最小のX値(生の値)
      cfg.x_max      = 4200;	// タッチスクリーンから得られる最大のX値(生の値)
      cfg.y_min      = 400;		// タッチスクリーンから得られる最小のY値(生の値)
      cfg.y_max      = 3400;	// タッチスクリーンから得られる最大のY値(生の値)
      cfg.pin_int    = 36;		// INTが接続されているピン番号 36
      cfg.bus_shared = false;	// 画面と共通のバスを使用している場合 trueを設定
      cfg.offset_rotation = 7;	// 表示とタッチの向きのが一致しない場合の調整 0~7の値で設定

// SPI接続の場合
      cfg.spi_host = VSPI_HOST;// 使用するSPIを選択 (HSPI_HOST or VSPI_HOST)
      cfg.freq = 10000000;   // SPIクロックを設定 10MHz
